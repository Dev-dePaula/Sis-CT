...
function carregarDashboard(u) {
    ...
    if (u.cargo === 'psicologa' || u.cargo === 'admin') {
        m.innerHTML += `<li onclick="mudarTela('psi-prontuario')">Prontuários</li>`;
    }
    if (u.cargo === 'social' || u.cargo === 'admin') {
        m.innerHTML += `<li onclick="mudarTela('social-documentos')">Documentação</li>`;
    }
    if (u.cargo === 'admin') {
        m.innerHTML += `<li onclick="mudarTela('admin-equipe')">Gerenciar Equipe</li>`;
    }
    carregarDadosBase();
}
...
function gerarTabelaLimpeza() {
    const dados = [
        ...
        {s: "COZINHA EXTERNA", t: "LIMPAR CHÃO, FOGÃO, PIA, ARMÁRIO E MESA"},
